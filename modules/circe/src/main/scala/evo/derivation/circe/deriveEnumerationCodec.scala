...
private object EnumerationCodec:
    inline def derived[A]: EnumerationCodec[A] =
        given config: Config[A] = Config.derived[A]
        summonFrom {
    ...
    private inline def deriveForSum[A](using config: => Config[A], mirror: Mirror.SumOf[A]): EnumerationCodec[A] = {
        val decoders              = summonSumOf[mirror.MirroredElemTypes].asInstanceOf[List[ProductImpl[A]]]
        ...
        new EnumerationCodec[A] {
            def apply(c: HCursor): Decoder.Result[A] =
                c.value.asString match
                    ...
                    case Some(value) =>
                        val decoded = LazyList
                            .from(decoders)
                            .map(_.apply(c))
                        decoded.collectFirst { case Right(decoded) =>
                            decoded
                        } match
